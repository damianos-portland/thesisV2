...
def replaceChars(text, dictionary={}):
    ...
    for key, value in list(replCharDict.items()):
    ...
    for root, dirs, files in os.walk(src, topdown=True):
        for name in files:
            if fnmatch.fnmatch(name, file_pattern):
                ...
                if os.name != 'posix':
                    for char in name:
                        for key, value in list(grToLat.items()):
                            if char.decode('windows-1253') == key:
    ...
    for dirpath, dirnames, filenames in os.walk(path, topdown=True):
        for name in filenames:
            if fnmatch.fnmatch(name, TXT_EXT):
                with open(os.path.join(dirpath, name) + "_pre_", "wt") as fout, open(os.path.join(dirpath, name),
                                                                                     "r") as fin:
                    mytext = fin.read()
...
def findDatesOfInterest(searchNodeElem, regexObj, dateName, author):
    ...
    for child in searchNodeElem.getchildren():
        childBytes = etree.tostring(child, pretty_print=True, encoding='UTF-8')
        if not childBytes:
            continue

        childText = childBytes.decode('utf-8')
        ...
        if DateOfInterest:
            ...
            # print elemIndex
            Date = datetime.date(int(DateOfInterest.group('yyyy')), months.get(DateOfInterest.group('mm'), 0),
                                 int(DateOfInterest.group('dd')))
            # print str(Date)
            DateStr = DateOfInterest.group('dd') + DateOfInterest.group('numSpecialLektiko') + DateOfInterest.group(
                'keno_1') + DateOfInterest.group('mm') + DateOfInterest.group('keno_2') + DateOfInterest.group(
                'yyyy')
            text_to_replace = "<date refersTo= " '"' + str(dateName) + '"' + " date = " + '"' + str(
                Date) + '"' + ">" + DateStr + "</date>"
...
def textToNumbering(text, dictionary):
    ...
    # Normalize the dictionary keys to str (decode bytes, leave str alone)
    unidict = {
        (k if isinstance(k, (bytes, bytearray)) else k): v
        for k, v in dictionary.items()
    }

    # If text is bytes, decode it once; if it's already str, leave it
    if isinstance(text, (bytes, bytearray)):
        text = text

    # Now apply your regex
    split = re.search(
        r'(?P<digit>\d*)(?P<space>\s*)(?P<lektiko>[Α-Ωα-ωάέήίόύώ]*)',
        text
    )
    if split:
        lekt = split.group('lektiko')
        if lekt:
            elemID.insert(0, unidict.get(lekt, '0'))
        dig = split.group('digit')
        if dig:
            elemID.insert(0, dig)

    return '_'.join(elemID) if elemID else '0'
